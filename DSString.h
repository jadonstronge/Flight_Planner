...
public:
    ...
    int find_first(char) const;
    // returns index of last occurrence of a character
    int find_last(char) const;

    // returns index of occurrence of a character after a specific index
    int find(int, char) const;

    // returns a pointer to the first occurrence of a substring in a char array
    const char* find(const DSString&) const;
